...
export function makeStockManager(config = {}) {
  ...
  return {
    ...
    async init(ns, ctrl) {
      if (!hasStockApis(ns)) {
        ctrl.stock = { enabled: false, reason: "NO_STOCK_APIS" };
        return;
      }

      const st = (await readJSON(ns, STATE_FILE)) ?? {};
    ...
    async tick(ns, ctrl, now) {
      ...
      const have4S = cfg.use4S && has4S(ns);
      ctrl.stock.lastMode = have4S ? "4S-forecast" : "trend-ema";

      // Build a snapshot and update price history
      const snapshot = symbols.map(sym => readSym(ns, sym, ctrl.stock, now, have4S, cfg));
      ...
      const desires = have4S
        ? computeForecastDesires(ns, snapshot, equity, cfg)
      ...
      for (const [sym, want] of rankDesires(desires)) {
        ...
        if (want.dir === "LONG") {
          ...
          if (typeof ns.stock.shortStock !== "function") continue; // shorts not available yet

          const shortMore = Math.max(0, want.targetShares - shortShares);
          if (shortMore > 0) {
            ns.stock.shortStock(sym, shortMore);
            setCooldown(ctrl.stock, sym, now, cfg.cooldownMs);
            actions++;
          }
        }
    ...
    status(ctrl) {
      ...
      const have4SNow = has4S(ctrl.nsForStatus ?? null) ? "yes" : "(unknown)";
...
function readSym(ns, sym, stockState, now, have4S, cfg) {
...
function estimateEquity(ns, snapshot) {
...
function computeForecastDesires(ns, snapshot, equity, cfg) {
  const scored = [];
  ...
  return capDesires(scored, equity, cfg);
}

function computeTrendDesires(ns, snapshot, equity, cfg) {
  const scored = [];
...
function capDesires(scored, equity, cfg) {
...
function rankDesires(desires) {
  return [...desires.entries()].sort((a, b) => (b[1].score ?? 0) - (a[1].score ?? 0));
}

function countOpenPositions(ns, symbols) {
  let n = 0;
  for (const sym of symbols) {
