...
export const HOME_RESERVED_RAM = 256;
...
export const HACK_CONFIG: HackConfig = {
    // Fraction of a server's maximum money to aim for before hacking.
    moneyThreshold: 0.70,
    //
    // Additional security above a server's minimum that we're willing to tolerate before weakening.
...
export const HACKNET_CONFIG: HacknetConfig = {
    // Don't spend more than this fraction of current money on a single purchase.
...
export interface PservConfig {
    ...
    maxSpendFraction: number;
    // Always try to keep minCashReserve in the bank.
...
export interface BatcherConfig {
    ...
    managerLoopMs: number;
    // Required scripts (paths)
    actionScripts: {
        hack: string;
        grow: string;
        weaken: string;
...
export const BATCHER_CONFIG: BatcherConfig = {
    // fractional portion of server.moneyMax to steal per batch (0.01 = 1%)
