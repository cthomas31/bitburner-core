 ...
 *
 * Returns the action with the shortest payback time, or null if nothing is worth buying.
 */
function findBestHacknetAction(ns: NS, ctx: HacknetContext): HacknetAction | null {
    const { hn, formulas, prodMult, constants, budget, maxPaybackSeconds } = ctx;

    let best: HacknetAction | null = null;

    const numNodes = hn.numNodes();

    // Helper to maybe update `best`
    const consider = (candidate: HacknetAction | null): void => {
        if (!candidate) return;
