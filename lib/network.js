...
/**
 * Find all servers where a backdoor can be installed.
 *
 * @param {NS} ns
 * @returns {Promise<string[]>} Array of hostnames where backdoors can be installed.
 */
export async function findBackdoorTargets(ns) {
    const targets = [];
    await explore(ns, {
        visit: (host) => {
            const s = ns.getServer(host);
            if (s.backdoorInstalled === false && s.requiredHackingSkill > 0)
                targets.push(host);
        }
    });
    return targets;
